...
bool IsMirrorBinaryTreeCore(BinaryTreeNode *pNode, BinaryTreeNode* pMirrorNode);
bool IsMirrorBinaryTree(BinaryTreeNode* pRoot){
	return IsMirrorBinaryTreeCore(pRoot, pRoot);
}
bool IsMirrorBinaryTreeCore(BinaryTreeNode *pNode, BinaryTreeNode* pMirrorNode){
	if (pNode == nullptr&&pMirrorNode == nullptr)
		return true;
	if (pNode == nullptr || pMirrorNode == nullptr)
		return false;
	if (pNode->m_nValue != pMirrorNode->m_nValue)
		return false;
	
	return IsMirrorBinaryTreeCore(pNode->m_pLeft, pMirrorNode->m_pRight) &&
		IsMirrorBinaryTreeCore(pNode->m_pRight, pMirrorNode->m_pLeft);
...
void Test(char* testName, BinaryTreeNode* pRoot, bool expected)
{
	...
	if (IsMirrorBinaryTree(pRoot) == expected)
		printf("Passed.\n");
	else
		printf("FAILED.\n");
}
